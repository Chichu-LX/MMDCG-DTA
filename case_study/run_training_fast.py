#!/usr/bin/env python3
"""Fast training with target standardization - no separate eval pass, higher LR."""
...
def main():
    print("=" * 60)
    print("Fast MMDCG-DTA Training for HIV-1 Protease")
    print("=" * 60)
    ...
    for epoch in range(1, 151):
        ...
        msg = (f"Epoch {epoch:03d} | Loss: {avg_loss:.4f} | "
               ...
               f"Pearson: {metrics['Pearson']:.4f} | Time: {elapsed:.0f}s")
        print(msg)
    ...
    print(f"Best epoch: {best_epoch}, Best Loss: {best_loss:.4f}, "
          f"Best RMSE: {best_loss**0.5 * target_std:.4f}")
    print(f"{'='*60}")
