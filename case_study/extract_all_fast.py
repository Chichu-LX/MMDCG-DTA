...
def parse_pdb_lines(pdb_path):
    """Parse PDB file, handling NMR multi-model structures (first model only)."""
    atom_lines = []
    het_lines = []
    in_first_model = True
    with open(pdb_path) as f:
        for line in f:
            ...
            if line.startswith("ENDMDL"):
                break  # Stop at end of first model
...
def main():
    ...
    for i, entry in enumerate(all_entries):
        ...
        try:
            # Print progress for every complex (with flush)
            print(f"  [{i+1}/{len(all_entries)}] {pid}...", end="", flush=True)

            ligand_content = extract_ligand_sdf(str(pdb_path), pid)
            if ligand_content is None:
                print(" SKIP (no ligand)")
                skip += 1
                continue

            # Use full protein for both protein and pocket (fast)
            protein_content = extract_protein_pdb(str(pdb_path))
            pocket_content = protein_content  # Same as protein for speed
            print(f" OK pKd={pKd:.2f}")
